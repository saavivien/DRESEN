...
import com.dresen.dresen.DaoImplement.AffectationDaoImpl;
...
//    }
     public static void main(String[] args) throws DataAccessException {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("dresenPU");
        EntityManager em = emf.createEntityManager();
        IAffectationDao iAffectationDao= new AffectationDaoImpl();
        //ICompteCourantDao iCompteCourantDao= new CompteDaoImpl();
        ((GenericDao) iAffectationDao).setManager(em);
        System.out.println("hello");
        
        EntityTransaction tx = em.getTransaction();
        tx.begin();
           Affectation affectation = iAffectationDao.findLastAffectationByIdAgent(229376L);
               System.out.println("intitulé de la structure:vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvssssssssssssssssssssssssssssssss"+affectation.getAgent().getNom());
           tx.commit();
    }
